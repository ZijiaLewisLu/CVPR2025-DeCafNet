        ...
        **kwargs
        ...
        for idx in range(arch[2]):
            if not pool_only:
                ...
                stride = 2 if idx > 0 else 1
                layer = MaskedConv1D(embd_dim, embd_dim, 3, stride, 1, groups=embd_dim, bias=False)

            self.branch.append(layer)
...
def make_video_net(opt):
    opt = deepcopy(opt)
    if opt['name'] == 'SGP':
        from ..tridet.backbones import SGPBackbone
        net = SGPBackbone(
            n_in=opt['in_dim'],
            n_embd=opt['embd_dim'],
            max_len=opt['max_seq_len'],
            sgp_mlp_dim=2048, # opt[''],
            n_embd_ks=opt[''],
            k=opt[''],
            arch=opt['arch'],
            scale_factor=opt['stride'],
            with_ln=opt[''],
            path_pdrop=opt['path_pdrop'],
            use_abs_pe=opt['use_abs_pe'],
        )
    else:
        net = backbones[opt.pop('name')](**opt)
    return net